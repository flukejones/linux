...
static int ally_x_parse_report(struct rog_ally_device *ally_x, struct hid_report *report,
		u8 *data, int size)
{
	struct hid_device *hdev = ally_x->hdev;
	...
	if (hdev->bus == BUS_USB && report->id == ALLY_X_INPUT_REPORT_USB &&
		...
		hid_err(hdev, "Unhandled reportID=0x%02X, bus=0x%02X, size=%d\n", report->id, hdev->bus, size);
		return -1;
	}
	...
	input_report_key(ally_x->gamepad, BTN_A,      in_report->buttons[0] & BIT(0));
	input_report_key(ally_x->gamepad, BTN_B,      in_report->buttons[0] & BIT(1));
	input_report_key(ally_x->gamepad, BTN_X,      in_report->buttons[0] & BIT(2));
	input_report_key(ally_x->gamepad, BTN_Y,      in_report->buttons[0] & BIT(3));
	input_report_key(ally_x->gamepad, BTN_TL,     in_report->buttons[0] & BIT(4));
	input_report_key(ally_x->gamepad, BTN_TR,     in_report->buttons[0] & BIT(5));
	input_report_key(ally_x->gamepad, BTN_START,  in_report->buttons[0] & BIT(6));
	input_report_key(ally_x->gamepad, BTN_SELECT, in_report->buttons[0] & BIT(7));
	input_report_key(ally_x->gamepad, BTN_THUMBL, in_report->buttons[1] & BIT(0));
	input_report_key(ally_x->gamepad, BTN_THUMBR, in_report->buttons[1] & BIT(1));
	input_report_key(ally_x->gamepad, BTN_MODE,   in_report->buttons[1] & BIT(2));

	if (in_report->buttons[2] == 0x07)
		input_report_abs(ally_x->gamepad, ABS_HAT0X, -1);
	else if (in_report->buttons[2] == 0x03)
		input_report_abs(ally_x->gamepad, ABS_HAT0X, 1);
	else
		input_report_abs(ally_x->gamepad, ABS_HAT0X, 0);

	if (in_report->buttons[2] == 0x05)
		input_report_abs(ally_x->gamepad, ABS_HAT0Y, 1);
	else if (in_report->buttons[2] == 0x01)
		input_report_abs(ally_x->gamepad, ABS_HAT0Y, -1);
	else
		input_report_abs(ally_x->gamepad, ABS_HAT0Y, 0);
...
static struct input_dev *rog_ally_allocate_input_dev(struct hid_device *hdev, const char *name_suffix)
{
	...
	hid_info(hdev, "Changing name '%s' to a more friendly name", hdev->name);
...
static int ally_x_play_effect(struct input_dev *idev, void *data, struct ff_effect *effect)
{
	...
	struct rog_ally_device *dev = hid_get_drvdata(hdev);

	if (effect->type != FF_RUMBLE)
		return 0;

	dev->ff_packet->ff.magnitude_strong = effect->u.rumble.strong_magnitude / 512;
	dev->ff_packet->ff.magnitude_weak = effect->u.rumble.weak_magnitude / 512;
	if (dev->output_worker_initialized)
		schedule_work(&dev->output_worker);
...
static struct input_dev *setup_capabilities(struct hid_device *hdev)
{
	...
	gamepad = rog_ally_allocate_input_dev(hdev, NULL);
	if (IS_ERR(gamepad))
...
{
	struct rog_ally_device *dev = container_of(work, struct rog_ally_device, output_worker);
	struct ff_report *report = dev->ff_packet;
	unsigned long flags;

	spin_lock_irqsave(&dev->lock, flags);
	report->ff.magnitude_left = report->ff.magnitude_strong;
	report->ff.magnitude_right = report->ff.magnitude_weak;
	asus_kbd_set_report(dev->hdev, (u8 *)report, sizeof(*report));
	spin_unlock_irqrestore(&dev->lock, flags);
}

static struct rog_ally_device *ally_x_create(struct hid_device *hdev)
{
	...
	max_output_report_size = sizeof(struct ally_x_input_report); // TODO: correct report
	report = devm_kzalloc(&hdev->dev, sizeof(report), GFP_KERNEL);
	if (!report)
		return ERR_PTR(-ENOMEM);

	// None of these bytyes will change for the FF command for now
...
static int rog_ally_raw_event(struct hid_device *hdev, struct hid_report *report,
		u8 *data, int size)
{
	struct rog_ally_device *dev = hid_get_drvdata(hdev);

	return ally_x_parse_report(dev, report, data, size);
}

static void rog_ally_remove(struct hid_device *hdev)
{
...
static int rog_ally_probe(struct hid_device *hdev, const struct hid_device_id *id)
{
	struct usb_interface *intf = to_usb_interface(hdev->dev.parent);
	struct usb_host_endpoint *ep = intf->cur_altsetting->endpoint;
	struct rog_ally_device *dev;
	int ret;
...
static struct hid_driver rog_ally_driver = {
	...
	.probe		= rog_ally_probe,
	.remove		= rog_ally_remove,
	.raw_event	= rog_ally_raw_event,
};
